...
def _payload_para_hash_conteudo(wa_doc: dict | None, df: pd.DataFrame, periodo_txt: str) -> bytes:
    payload = {
        "wa_doc": wa_doc or {},
        "periodo": periodo_txt,
...
def gerar_relatorio_html_txt_docx(df_base, df_filtrado, incluir_graficos, wa_doc=None):
    ...
    content_payload = _payload_para_hash_conteudo(wa_doc, df, periodo_txt)
    ...
    # >>> Espaçamento extra de 2 linhas entre o título e a próxima seção
...
# ---- PDF (sem metadados; título = Relatório do WhatsApp) ----
...
if uploaded_files:
    ...
    if dfs:
        ...
        with aba8:
            st.subheader("📝 Gerar Relatório do WhatsApp (hash no final)")
